...
const FIXTURES: { label: string; script: string }[] = [
  ...
  { label: 'maintenance / bracket drop', script: 'db.items["drop"]()' },
  { label: 'mutation / bracket deleteMany', script: "db.logs['deleteMany']({})" },
  { label: 'mutation / bracket coll + bracket op', script: 'db["logs"]["deleteMany"]({})' },
  { label: 'maintenance / getCollection bracket op', script: 'db.getCollection("c")["drop"]()' },
  { label: 'null / bracket op in string', script: 'const s = "db.x[\'drop\']()"; print(s);' },
  { label: 'null / bracket op on non-db', script: "[1,2,3]['find'](x => x > 1)" },
